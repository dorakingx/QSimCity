...
| --- | --- | --- | --- |
| W1.1 | Terrain heightfield: every building, road, prop, vehicle, and pedestrian sits on `terrainHeight(x,z)`; no floating or buried geometry | `packages/world/test/terrain.test.ts` grounding property test | PASS |
| W1.2 | Water present at both waterfronts with quays and vessels; water level consistent with terrain | `packages/world/test/terrain.test.ts`; street/overview screenshots | PASS |
| W1.3 | Hierarchical road network (boulevard, collector ring, local streets) with connected graph — every district reachable | `packages/world/test/roads.test.ts` connectivity test | PASS |
| W1.4 | Every district subdivided into blocks/parcels; zero empty districts; parcels non-overlapping | `packages/world/test/blocks.test.ts` coverage + overlap tests | PASS |
| W1.5 | Buildings vary: no two adjacent buildings share identical footprint, height, and facade style | `packages/world/test/architecture.test.ts` variation test | PASS |
| W1.6 | Buildings meet the ground and do not intersect roads or each other | `packages/world/test/architecture.test.ts` collision/clearance tests | PASS |
| W1.7 | Facade texture atlas generated procedurally and deterministically (same seed, same pixels) | `packages/visual-engine/test/facades.test.ts` determinism test | PASS |
| W1.8 | At least three enterable interiors reachable in walk mode with consoles inside | `packages/world/test/interiors.test.ts`; walk-mode screenshot | PASS |
| W1.9 | Street furniture (lamps, trees, benches, district props) present and instanced | `packages/world/test/props.test.ts`; draw-call budget in FPS report | PASS |
| W1.10 | No `Math.random` in world layout or rendering paths | `packages/visual-engine/test/determinism.test.ts` source scan | PASS |
...
| ID | Criterion | Evidence | Status |
| --- | --- | --- | --- |
| W4.1 | All twelve stage behaviors of spec section 5.1 implemented and driven only by trace events | `packages/world/test/playback.test.ts` stage-behavior derivations | PASS |
| W4.2 | Persistent logical-qubit identities distinct from physical qubits; mapping at tick derived from layout+SWAP events | `packages/world/test/mapping.test.ts` (property: matches compiler final layout) | PASS |
| W4.3 | SWAP events visibly exchange logical banners between pylons at the same tick the inspector updates | `packages/world/test/mapping.test.ts`; E2E swap scenario | PASS |
| W4.4 | Harbor container stacks equal live measured counts at every tick | `packages/world/test/playback.test.ts` histogram derivation | PASS |
| W4.5 | Scrubbing to any tick reproduces identical city state (pure derivation, no hidden animation state affecting meaning) | `packages/world/test/playback.test.ts` determinism test | PASS |
| W4.6 | City Legend lists every animated entity class with meaning, trigger, and source/certainty badge | `packages/ui/test/components.test.tsx` legend completeness test | PASS |
| W4.7 | Ideal vs physical vs noisy remain three labelled result classes throughout | existing `physical-execution.test.ts` + results UI tests | PASS |

## W5. Cameras and input

| ID | Criterion | Evidence | Status |
| --- | --- | --- | --- |
| W5.1 | Orbit, top, fly, and 1.7 m walk modes all smooth (damped) and collision-safe | `packages/visual-engine/test/cameras.test.ts` | PASS |
| W5.2 | Full touch operation: orbit/pinch plus on-screen controls for walk/fly | `tests/e2e/mobile.spec.ts` touch tests | PASS |
| W5.3 | Full keyboard operation for all modes | `tests/e2e/smoke.spec.ts` keyboard test; cameras tests | PASS |

## W6. Child-friendly learning path

| ID | Criterion | Evidence | Status |
| --- | --- | --- | --- |
| W6.1 | Picture-based first-run onboarding with three illustrated entries; persists; never blocks return visits | `packages/ui/test/onboarding.test.tsx`; E2E first-run test | PASS |
| W6.2 | Drag-and-drop circuit builder (mouse + touch), undo/redo/reset, compiles through the standard pipeline | `packages/ui/test/builder.test.tsx`; E2E touch builder test | PASS |
| W6.3 | One-click Bell mission template | mission-1 E2E test | PASS |
| W6.4 | Pause/step/rewind/undo/reset contextual controls in missions | `packages/ui/test/missions.test.tsx` | PASS |
| W6.5 | At least six playable missions with machine-checkable completion and immediate feedback | `packages/ui/test/missions.test.tsx` (every mission completes against a real trace) | PASS |
| W6.6 | Mission 1 completable by a first-time learner from on-screen guidance alone (no Qiskit knowledge) | `tests/e2e/missions.spec.ts` scripted naive-user walkthrough | PASS |
| W6.7 | Child/beginner/expert explanation levels across narration, missions, tour, inspector, legend | `packages/ui/test/explanations.test.ts` completeness matrix | PASS |
| W6.8 | Pre/post assessment (5 picture questions), local-only, growth-framed, clearable | `packages/ui/test/assessment.test.tsx` | PASS |
| W6.9 | Missions and builder fully available in Accessible 2D mode | `tests/e2e/fallback.spec.ts` extension | PASS |
