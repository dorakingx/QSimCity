...
## What actually executes, and the three results

A device run compiles the program and then executes **the compiled circuit**,
not the original one. That distinction is the whole point of the compiler
districts, and getting it wrong makes them decorative: an earlier version of
this pipeline compiled the circuit and then simulated the logical one, so a
deliberately bad initial layout produced exactly the same distribution as a
good one.

Three results are therefore kept apart, because they answer different
questions and conflating them destroys the answer to all three:

| Result | Circuit | Noise | Question it answers |
| --- | --- | --- | --- |
| Logical reference | As written | None | What does this program mean? |
| Physical ideal | Compiled | None | Did compilation preserve that meaning? |
| Physical noisy | Compiled | Configured model | What did the device do to the circuit that ran? |

The physical ideal result must reproduce the logical reference. When it does
not, compilation changed the program — that is a bug in the compiler, not a
property of the hardware model, and tests assert the agreement directly.

The physical noisy result is where inserted SWAPs become visible. Each SWAP
the router adds is three more two-qubit operations for the noise model to act
on, so routing-heavy compilations drift further from the reference than
routing-light ones. This is a **modeled** consequence of a simplified noise
model, labeled `SAMPLED`, and it is not a fidelity and not a measurement of
any real device.

Execution events record the qubits that ran, which on a compiled circuit are
physical device qubits. They carry `physicalQubits`; the logical qubit each one
started as is recorded separately as `logicalOrigin`, because routing SWAPs
permute that mapping as the circuit runs. The physical identity is exact at
every moment; the logical one is only true at the start.
