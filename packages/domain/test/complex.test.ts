...
describe('matMul', () => {
  it('multiplying by identity is a no-op', () => {
    const a = mat([[0.3, 0.1], [0.2, -0.4], [0.5, 0.6], [-0.7, 0.8]]);
  ...
  it('computes complex products correctly', () => {
    // [[i, 0], [0, i]] squared = -I
    const iI = mat([[0, 1], [0, 0], [0, 0], [0, 1]]);
    const sq = matMul(iI, iI, 2);
    const negI = mat([[-1, 0], [0, 0], [0, 0], [-1, 0]]);
    expect(matMaxDiff(sq, negI)).toBeLessThan(1e-12);
  });

  it('is associative', () => {
    const a = mat([[1, 1], [0, 0], [0, 0], [1, -1]]);
    const b = mat([[0, 1], [1, 0], [1, 0], [0, -1]]);
    const c = mat([[0.5, 0], [0, 0.5], [0.5, 0], [0, -0.5]]);
